...
各GIFは実際のROMのリプレイから復元した8秒・25fps・無音映像です。開発用の自動操作による記録で、プレイヤーによるプレイではありません。ゲーム速度の加工や無敵化はしていません。保存したVRAMと各時点の実パレットから画像化しています。

Each silent 8-second, 25 fps GIF uses native emulator VRAM and palette captures from a deterministic replay generated by developer automation, not a player recording. No invincibility or artificial playback speed-up is used.
...
- 現状は16384入力までです（30更新/秒なら約9分6秒）。STのマッパRAMを活用する長時間記録は未実装です。
