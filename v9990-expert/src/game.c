...
u8 score_digits[8];
u8 graze_batch;
u8 disk_status, disk_old, replay_source;
u16 spawn_attempted,spawn_dropped;
...
u8 expert_spawn(int x,int y,signed char dx,signed char dy,u8 c) {
 ...
 for(i=0;i<192;++i)if(!bullets[i].live){
  ...
  bullets[i].live=1;bullets[i].graze=0;bullets[i].color=c;patterns_slot_reset(i);return i;
 }
 ++spawn_dropped;return 255;
}
static void reset_run(void) {
 music_init();patterns_reset();spawn_attempted=0;spawn_dropped=0;
...
static void step(u8 k) {
 ...
 level=tick/360;if(level>12)level=12;
 if(spark)--spark;
 /* Additional motifs overlap the continuously running original barrage. */
 patterns_step(level);
 period=35-level*2;
 if(tick%period==0) {
  x=20+rnd()%216;
  expert_spawn(x,20,(player_x-x)/12,16+level,10);
 }
 /* Rotating 16-spoke rosettes; phase shifts four fine steps per wave. */
 if(tick%60==0){
  a=(tick/15)&63;
  for(i=0;i<16;++i)expert_spawn(128,52,vx[(i*4+a)&63],vy[(i*4+a)&63],4);
 }
 /* Mirrored fans keep a legible symmetry while their phase slowly changes. */
 if(level>=1 && tick%54==18){
  a=(tick/54)%9;
  for(i=0;i<7;++i){
   expert_spawn(48,28,vx[(i*3+54+a)&63],vy[(i*3+54+a)&63],9);
   expert_spawn(208,28,-vx[(i*3+54+a)&63],vy[(i*3+54+a)&63],9);
  }
 }
 /* Opposite-turning spiral streams, deliberately separated from fan bursts. */
 if(level>=3 && tick%8==0){
  a=(tick/4)&63;
  expert_spawn(80,60,vx[a],vy[a],4);
  expert_spawn(176,60,-vx[a],vy[a],9);
 }
 /* Alternating diagonal waves cross from the side edges after level 4. */
 if(level>=4 && tick%96==48){
  a=(tick/96)&1;
  for(i=0;i<3;++i){
   expert_spawn(8,48+i*32,20,a?7:-7,12);
   expert_spawn(248,64+i*32,-20,a?-7:7,12);
  }
 }
 ...
 if(tick==(replay && saved_result==2 ? recorded : REPLAY_CAPACITY))finish(2);
}
...
/* Labels are rasterized once, then copied with one V9990 command. */
static void expert_labels(void){
 gfx_fill(0,600,256,48,0);
 text("EXPERT",0,600);
 text("FROM ABOVE",0,612);text("FROM BELOW",0,624);text("FROM SIDES",0,636);
}
static void draw(void) {
 ...
 gfx_blit(0,600,104,base+4,48,10,1);
 if(mode==1 && pattern_active && pattern_age<32){
  gfx_blit(0,612+(u16)pattern_hint*12,160,base+4,80,10,1);
  for(i=0;i<pattern_emitter_count;++i){
   x=pattern_emitter_x[i];y=pattern_emitter_y[i];
   gfx_line(x-3,base+y,x,base+y-3,6);
   gfx_line(x,base+y-3,x+3,base+y,6);
   gfx_line(x+3,base+y,x,base+y+3,6);
   gfx_line(x,base+y+3,x-3,base+y,6);
  }
 }
...
void main(void) {
 u8 *p;u8 k,edge;
 for(p=(u8*)0xC800;p<(u8*)0xD400;++p)*p=0;
 gfx_init();title_upload();
 /* Startup art stays in ROM while cartridge page 1 is still mapped. */
 *(volatile u8*)0x6800=5;
 aura_upload();background_init();
 disk_init();hardware_upload();
...
{u8 j;gfx_fill(0,528,80,12,0);for(j=0;j<10;++j)glyph(digits[j],j*8,528,7);}
 ...
 while(1){
  ...
  if(mode!=1){
   ...
   if(de&2){disk_status=disk_load()+1;if(disk_status==1){
    u8 si;u32 n=saved_score;replay_source=2;replay=0;replay_match=0;score=saved_score;result=saved_result;
    for(si=0;si<8;++si){score_digits[si]=n%10;n/=10;}
   }else if(!recorded)replay_source=0;}
   if(edge&INPUT_BOMB){if(recorded){replay=1;replay_match=0;reset_run();}else disk_status=7;}
   else if(edge&INPUT_FIRE){replay=0;recorded=0;replay_source=1;disk_status=0;reset_run();}
